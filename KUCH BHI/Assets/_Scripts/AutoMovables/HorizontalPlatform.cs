...
public class HorizontalPlatform : MonoBehaviour
{
    ...
    [SerializeField] private float moveDistance = 3f; // Distance to move left and right
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            ...
            // Fetch platform parent scale (e.g. x = 2.0, y = 0.5, z = 0)
            Vector3 parentScale = transform.localScale;

            // Prevent division by zero for Z-axis scale
            float safeZ = parentScale.z != 0 ? parentScale.z : 1f;

            // Compensate local scale so world scale remains crisp (0.1 x 0.1 x 0.1)
    ...
    private void OnCollisionExit2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            ...
            // Restore un-parented base scale while preserving facing direction
            float facingDir = Mathf.Sign(collision.transform.localScale.x);
            collision.transform.localScale = new Vector3(baseScale * facingDir, baseScale, baseScale);
        }
    }

    // Visualize the movement line in the Scene view
